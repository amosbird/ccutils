...
public:
    ...
    Flags(std::initializer_list<char const* const> pre_reg_names) { add_params(pre_reg_names); }

    Flags(const char* const argv[], int mode = PREFER_FLAG_FOR_UNREG_OPTION) { parse(argv, mode); }

    Flags(int argc, const char* const argv[], int mode = PREFER_FLAG_FOR_UNREG_OPTION) {
...
inline void Flags::parse(const char* const argv[], int mode) {
    int argc = 0;
...
inline void Flags::parse(
    ...
    for (auto i = 0u; i < args_.size(); ++i) {
        ...
        if (1 == (args_[i].size() - name.size()) && // single dash
            ccutils::Flags::SINGLE_DASH_IS_MULTIFLAG & mode && // multi-flag mode
        ...
        assert(!(mode & ccutils::Flags::PREFER_FLAG_FOR_UNREG_OPTION)
            || !(mode & ccutils::Flags::PREFER_PARAM_FOR_UNREG_OPTION));

        bool preferParam = mode & ccutils::Flags::PREFER_PARAM_FOR_UNREG_OPTION;
...
inline string_stream Flags::bad_stream() const {
    string_stream bad;
...
inline bool Flags::is_number(std::string const& arg) const {
...
inline bool Flags::is_option(std::string const& arg) const {
    assert(0 != arg.size());
    if (is_number(arg))
        return false;
    return '-' == arg[0];
...
inline std::string Flags::trim_leading_dashes(std::string const& name) const {
...
inline bool ccutils::Flags::got_flag(std::string const& name) const {
    return flags_.end() != flags_.find(trim_leading_dashes(name));
}

//////////////////////////////////////////////////////////////////////////

inline bool ccutils::Flags::is_param(std::string const& name) const {
    return registeredParams_.count(name);
}

//////////////////////////////////////////////////////////////////////////

inline bool Flags::operator[](std::string const& name) const { return got_flag(name); }

//////////////////////////////////////////////////////////////////////////

inline bool Flags::operator[](std::initializer_list<char const* const> init_list) const {
    return std::any_of(
...
inline std::string const& Flags::operator[](size_t ind) const {
...
inline string_stream Flags::operator()(std::string const& name) const {
...
inline string_stream Flags::operator()(std::initializer_list<char const* const> init_list) const {
...
template <typename T> string_stream Flags::operator()(std::string const& name, T&& def_val) const {
    auto optIt = params_.find(trim_leading_dashes(name));
    if (params_.end() != optIt)
        return string_stream(optIt->second);
...
inline string_stream Flags::operator()(size_t ind) const {
...
template <typename T> string_stream Flags::operator()(size_t ind, T&& def_val) const {
    if (pos_args_.size() <= ind) {
        std::ostringstream ostr;
        ostr << def_val;
...
inline void Flags::add_param(std::string const& name) {
    registeredParams_.insert(trim_leading_dashes(name));
}

//////////////////////////////////////////////////////////////////////////

inline void Flags::add_params(std::initializer_list<char const* const> init_list) {
    for (auto& name : init_list)
        registeredParams_.insert(trim_leading_dashes(name));
}
}
